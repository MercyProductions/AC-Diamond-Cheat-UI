...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(107, 105, 106, 255), 4.f);
...
bool ImGui::Defaults(const char* label, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    {
        if (label == "Legit")
            window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(86, 124, 83), 4.f);
        else if (label == "Semi Legit")
            window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(145, 124, 95), 4.f);
        else
            window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(95, 55, 47), 4.f);
    }
    else
    {
        if (label == "Legit")
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(86, 124, 83), 4.f, 0, 1.5f);
        else if (label == "Semi Legit")
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(145, 124, 95), 4.f, 0, 1.5f);
        else
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(95, 55, 47), 4.f, 0, 1.5f);
    }
...
bool ImGui::Switcher(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    RenderNavHighlight(bb, id);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(107, 105, 106, 255), 4.f, 0, 1.5f);
    ...
    PushFont(icons);
    if (label == "A")
        RenderTextClipped(bb.Min + style.FramePadding - ImVec2(0, 5), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    else
        RenderTextClipped(bb.Min + style.FramePadding - ImVec2(-3, 5), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    PopFont();
...
bool ImGui::Tabs(const char* label, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ...
    if (selected)
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(107, 105, 106, 255), 4.f, ImDrawFlags_RoundCornersTop);
    else
        window->DrawList->AddRect(bb.Min, bb.Max, ImColor(107, 105, 106, 255), 4.f, ImDrawFlags_RoundCornersTop, 1.5f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    GetWindowDrawList()->AddRect(check_bb.Min + ImVec2(w - 10, 1), check_bb.Max + ImVec2(w, 1), ImColor(107, 105, 106, 255), 4.f, ImDrawFlags_None, 1.5f);

    GetWindowDrawList()->AddRect(check_bb.Min + ImVec2(w - 280, 1), check_bb.Max + ImVec2(w - 270, 1), ImColor(107, 105, 106, 255), 4.f, ImDrawFlags_None, 1.5f);

    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x - square_sz, check_bb.Min.y), GetColorU32(ImGuiCol_Text), label);

    if (*v)
        GetWindowDrawList()->AddText(check_bb.Min + ImVec2(w - 138, 1), ImColor(48, 92, 39, 255), "On");
    else
        GetWindowDrawList()->AddText(check_bb.Min + ImVec2(w - 138, 1), ImColor(128, 42, 41, 255), "Off");

    PushFont(icons);
    GetWindowDrawList()->AddText(check_bb.Min + ImVec2(w - 271, 0), GetColorU32(ImGuiCol_Text), "A");

    GetWindowDrawList()->AddText(check_bb.Min + ImVec2(w + 2, 0), GetColorU32(ImGuiCol_Text), "B");
    PopFont();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(365, 0));
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        RenderTextClipped(bb.Min + style.FramePadding, ImVec2(bb.Max.x, bb.Max.y), preview_value, NULL, NULL, ImVec2(0.5, 0.5));
    }
    if (label_size.x > 0)
        RenderText(ImVec2(bb.Max.x + style.ItemInnerSpacing.x - 580, bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(107, 105, 106, 255), 4.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, ImColor(107, 105, 106, 255), 4.f);
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 580, frame_bb.Min.y + style.FramePadding.y - 1), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(320, 0));
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() + 90, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    if (is_multiline)
    {
        ...
        PushStyleColor(ImGuiCol_ChildBg, ImVec4(ImColor(107, 105, 106, 255)));
        PushStyleVar(ImGuiStyleVar_ChildRounding, style.FrameRounding);
        PushStyleVar(ImGuiStyleVar_ChildBorderSize, style.FrameBorderSize);
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0)); // Ensure no clip rect so mouse hover can reach FramePadding edges
        bool child_visible = BeginChildEx(label, id, frame_bb.GetSize() - ImVec2(7,0), true, ImGuiWindowFlags_NoMove);
        PopStyleVar(3);
        PopStyleColor();
        if (!child_visible)
        {
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(107, 105, 106, 255), true, style.FrameRounding);
    }
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 625, frame_bb.Min.y + style.FramePadding.y), label);
